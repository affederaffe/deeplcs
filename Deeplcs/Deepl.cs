...
namespace Deeplcs
{
    public class Deepl
    {
        ...
        public static string[] Translate(string targetLang, string[] stringArray, string[] srcTextArray = null, string[] targetTextArray = null, string srcLang = "auto", bool headless = true)
        {
            string[] paths = Json();
            ...
            options.AddArguments("--disable-extensions", "--window-size=1920,1080", "--start-maximized");
            if (headless)
            {
            ...
            if (srcLang != "auto")
            {
                var srcLanguageMenuButtonElement = driver.FindElement(By.XPath("//*[@id='dl_translator']/div[1]/div[3]/div[1]/div/button"));
                srcLanguageMenuButtonElement.Click();
                Thread.Sleep(500);
                var srcLanguageButtonElements = driver.FindElements(By.CssSelector(string.Format("button[dl-lang = {0}", srcLang)));
                srcLanguageButtonElements[0].Click();
            }
            var targetLanganguageMenuButtonElement = driver.FindElement(By.XPath("//*[@id='dl_translator']/div[1]/div[4]/div[1]/div[1]/div[1]/button"));
            targetLanganguageMenuButtonElement.Click();
            Thread.Sleep(500);
            var targetLanguageButtonElements = driver.FindElements(By.CssSelector(string.Format("button[dl-lang = {0}", targetLang)));
            targetLanguageButtonElements[1].Click();
